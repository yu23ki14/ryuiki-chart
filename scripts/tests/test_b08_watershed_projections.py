...
def test_representative_requires_dated_population_and_moved_records_are_measured(tmp_path):
    ...
    counts = b08.build_watershed_projections(v2_db, registry_db, out, decl)

    assert counts["org_watershed_year"] == 3  # (W2,2020) (W2,2021) (W2,2022)
    assert counts["memo_moved_records"] == 1
    assert counts["ws_to_ws"] == 0
    assert counts["v1_assigned_exact_unassigned"] == 1
    assert counts["v1_unassigned_exact_assigned"] == 0
    assert counts["memo_mixed_buckets"] == 1
    assert counts["org_watershed_year_keys_changed_vs_exact"] == 1
...
def test_floor_half_up_boundary_merges_into_same_bucket(tmp_path):
    ...
    と同じ「0.001度単位で最も近い整数に丸める」規則）で、生の座標が違っても
    同じ0.001度バケットに入る2点が、実際に1つのバケットとして扱われることを
    確認する（バケットの境界は `math.floor(x*1000+0.5)` で機械的に決まる
    ——`round()`（銀行丸め）や単純な切り捨てではなく、常に「.5 以上は
    繰り上げ」になることをこのテストで固定する）。
    """
    import math

    bx_target = math.floor(139.0100 * 1000 + 0.5)
    lon_b, lon_c = 139.0100, 139.0104
    assert math.floor(lon_c * 1000 + 0.5) == bx_target  # 同じバケットに入る前提の確認
    occurrence_rows = [
        occurrence_row_at(
    ...
    assert counts["ws_to_ws"] == 1  # C は B と別バケット扱いなら moved にならないはず
