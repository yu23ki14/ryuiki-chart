...
- 状態: 承認済 / 日付: 2026-09-08
- 関連: ADR-0007（observation）, ADR-0008（時間）, ADR-0010（指標）, ADR-0011（キューブ）, ADR-0016（移行計画）

**2026-09-24 追記（ADR-0009 決定4）**: 本 ADR の決定2が足した次元キーから
`imputation` を外した（13列→12列）。`imputation` はキューブのキー（次元）
ではなく `value_zero`/`value_lod` という**値の列の宣言**であることが、
`zero`/`lod` 併記の実装で判明した——「入力側の統計量」（`obs_stat`）や
「日付の精度」（`value_grain`）のようにセルを**分割する**軸とは性質が違い、
`imputation` は同じセルの中で**どの値を読むか**を選ぶだけの軸だった。詳細は
ADR-0009 決定4・ADR-0011 の2026-09-24追記。

**2026-09-15 追記（センサーの縦線・ADR-0024）**（D-2: 以下は
`scripts/b04_build_cube.py` のモジュール docstring「センサーの縦線 設計 v2 T4」節の要約。
決定の要点はここにも残すが、実装の詳細・最新の状態はそちらを正とする）: 観測はそれぞれ
自分の `period_grain` の格に入る——`day` は日次セル、`hour`/`instant` は日次セルに積み上げ
（日付は区間の始まり。ADR-0024）、
...
（日次セルから作る側）は `stat='mean'` の日次セルだけを使い、`input_grain` は日次セルから
**引き継ぐ**（`'day'` 直書きをやめた。実装は当初この日次セル集合を `cube_day` という一時
テーブルに丸ごとコピーしてから読んでいたが、C-1 の効率化で `observation_agg` を直接
`WHERE grain='day' AND stat='mean'` で絞り込む形に変わった——引き継ぐという決定自体は
変わらない）。**既知の制約**: 月・年の格の鍵に
