...
**2026-09-25 改訂**: `/code-review` の指摘を受け、「承認済」とした14本のうち0007・0011・0019
の3本、および0006・0004・0010の`一部未実装`注記に、決定の各項目まで遡って読み直しても
本当に実装されているかを再確認した。結果、ADR-0011は決定の中核（宣言的集計定義）が
実装されておらず`提案中`に差し戻した。0004・0006・0007・0010・0019は追加の未実装項目が
見つかったため注記を拡充した。詳細は各行と、対応する ADR ファイルの状態行・2026-09-25追記を
参照。
...
| [0011](0011-aggregation-cube.md) キューブ | 提案中（2026-09-25 `/code-review`指摘により`承認済`から差し戻し） | 決定文（`docs/adr/0011-aggregation-cube.md:94-95`）は「単一キューブ**＋**宣言的集計定義（YAML）で表す。テーブルを手書きしない」の2要素からなる。前半（`observation_agg`/`occurrence_agg`という単一物理表への統合、`scripts/b04_build_cube.py:389` `build_cube()`・`scripts/b07_build_occurrence_cube.py`）は実装済みで、`docs/plans/PHASE_B_RECONCILIATION.md:556,885-894`「12. 全体の合格」がv1派生33テーブル全数の再現（一致25/宣言済み差分8/不一致0）を確認している。しかし後半（宣言的集計定義）は、リポジトリに`aggregations/`ディレクトリが存在せず（`find . -iname aggregations`が0件）、`b04_build_cube.py`はYAMLパーサ呼び出し0件（`grep -c "yaml.safe_load\|yaml.load" scripts/b04_build_cube.py`が0）で、GROUP BY・出典ごとの分岐をPythonコードに直接書いている。`imputation='lod'`・`roll_up_to`もscripts/に実装なし、`coverage_ratio`は`scripts/b04_build_cube.py:109-110`「この縦線で埋めないもの」と明記。ゲートが検証したのは「v1と数値が一致すること」（ADR-0016の受け入れ基準）であって「宣言的な集計定義で新しい集計軸をコード変更なしに追加できること」（本ADR合格条件2、`docs/adr/README.md`§2）ではないため、`承認済（一部未実装）`ではなく`提案中`と判定した |
