...
パーティション列）で表すのであって、レジストリの ID に埋めない」を `place.region_id` 列に
具体的に適用し、「common ならNULL、それ以外は scope 自身」という不変条件にした。
詳細は [ADR-0022](0022-place-region-scope.md)。

**2026-09-25 追記**: 規約1「`local_key` は出典の識別子をそのまま使ってよいが、必ず名前空間を
前置する」の例（`env-pubwater-0142`）自体が、出典の名前空間とローカルキーの区切りにどちらも
`-` を使っており、機械的に分解できない（実例: `jp-14:place:site.jiban-chinka-12-1`。
