...
- 状態: 提案中（2026-09-25 状態再確認: 決定文は「単一キューブ**＋**宣言的集計定義（YAML）で
  表す。テーブルを手書きしない」という2つの要素からなる。前半（`observation_agg`/
  `occurrence_agg`という単一物理表への統合）は`scripts/b04_build_cube.py`/
  `scripts/b07_build_occurrence_cube.py`で実装され、`docs/plans/PHASE_B_RECONCILIATION.md`
  §12でv1派生33テーブル全数の再現が確認されている。しかし後半（宣言的集計定義）は
  リポジトリに`aggregations/`ディレクトリが存在せず、`b04_build_cube.py`はYAMLを一切
  読まずGROUP BY・列挙をPythonコードに直接書いている（`imputation='lod'`・`coverage_ratio`
  ・`roll_up_to`もscripts/に実装なし、`coverage_ratio`は`b04_build_cube.py`内で「この縦線で
  埋めないもの」と明記）。ゲートが確認したのは「v1と数値が一致すること」（ADR-0016の
  受け入れ基準）であって「宣言的な集計定義で新しい集計軸をコード変更なしに追加できること」
  （本ADRの合格条件2、README §2）ではない。決定の核である「テーブルを手書きしない」という
  設計そのものが実現していない——今の実装は「1個の巨大な手書きテーブルに集約した」段階で
  あり、ADRが約束した宣言的フレームワークには至っていないため、`承認済（一部未実装）`ではなく
  `提案中`に戻す） / 日付: 2026-09-06
...
はこの区別の実例（上の骨格の例には挙げているが、実際にはキューブに入らない）——
理由は上記2026-09-22追記・下記「33テーブルの行き先」表を参照。）
