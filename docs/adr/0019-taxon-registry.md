...
- 状態: 承認済（一部未実装: 決定3〔ファクトはカテゴリ文字列を持たない〕に反し`occurrence`が
  `red_list_category`等の原表記を保持、決定5「最新評価をビューで表す」のVIEW未実装、
  `accepted_taxon_id`全行NULL〔#8〕、`taxon_name`テーブル未実装） / 日付: 2026-09-06
