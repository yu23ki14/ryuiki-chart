    ...
    it(table, () => {
      expect(caveatsForTables([table])).toMatchInlineSnapshot(`
        [
          ...
          {
            "key": "censored",
            "text": "全体の約24%は定量下限未満（原表記が「<0.5」など）。この画面の値は定量下限未満を 0 とみなして集計している。折れ線では中抜きの点で示し、「0 が観測された」とは読まないこと。",
          },
          {
            "key": "duplicates",
          ...
          {
            "key": "aboveLod",
            "text": "透明度の定量上限超え（原表記が「>1.4」〜「>28」など、26行）は、上限がどこまでか分からないという性質上、集計方法によらず値に含められない。件数（n）にも入らないため、他の期間・地点と単純に比較しないこと。",
          },
        ]
      `);
    });
  ...
  expect(CAVEAT_TEXT).toMatchInlineSnapshot(`
    {
      ...
      "censored": "全体の約24%は定量下限未満（原表記が「<0.5」など）。この画面の値は定量下限未満を 0 とみなして集計している。折れ線では中抜きの点で示し、「0 が観測された」とは読まないこと。",
