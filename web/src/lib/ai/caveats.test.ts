    ...
    it(table, () => {
      expect(caveatsForTables([table])).toMatchInlineSnapshot(`
        [
          ...
          {
            "key": "censored",
            "text": "全体の約24%は定量下限未満（原表記が「<0.5」など）。値の扱いは代入方式によって変わる（0とみなす方式／定量下限値とみなす方式）。折れ線では中抜きの点で示し、いずれの方式でも「0 が観測された」とは読まないこと。",
          },
          {
            "key": "duplicates",
...
it("CAVEAT_TEXT — 全キー・全本文を固定する", () => {
  expect(CAVEAT_TEXT).toMatchInlineSnapshot(`
    {
      "censored": "全体の約24%は定量下限未満（原表記が「<0.5」など）。値の扱いは代入方式によって変わる（0とみなす方式／定量下限値とみなす方式）。折れ線では中抜きの点で示し、いずれの方式でも「0 が観測された」とは読まないこと。",
